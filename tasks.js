...
// firstArray.forEach(commonElement => {if (secondArray.includes(commonElement)) {
//       commonElements.push(commonElement);
// };
// }
// )
...
//     if (numbers[i] % 2 === 0) {
//       const res = numbers[i] + value;
//       oddNumbers.push(res);
//     }
//     else oddNumbers.push(numbers[i])
//   }
//   return oddNumbers;
//   // Пиши код выше этой строки
// }
...
// ============TASK14===========
// // Пиши код ниже этой строки
// const getUserEmails = users => {return users.map(user => {return user.email})
...
// ];
// // Пиши код ниже этой строки
// const allGenres = books.flatMap((book) => {return book.genres});
// const uniqueGenres = allGenres.filter((genre, idx, arr) => {return arr.indexOf(genre) === idx});
...
//   const uniqueFriends = allFriends.filter((uniqueFriend, idx, arr)=> {return arr.indexOf(uniqueFriend) === idx})
...
//   const userTargeting = users.find(user => user.email === email);
...
// const totalPlayTime = playtimes.reduce((akk, time) => akk+time);
...
//   return [...users].sort((firstUser, secondUser) => {return firstUser.name.localeCompare(secondUser.name)}) 
// };
// // Пиши код выше этой строки
...
// const names = [...books]
// 				.filter(valideBook => valideBook.rating > MIN_BOOK_RATING)
...
//      .filter((uniqueFriend,idx,array) => array.indexOf(uniqueFriend) === idx)
//      .sort((uniqueFriendA,uniqueFriendB) => uniqueFriendA.localeCompare(uniqueFriendB))
// };
// // Пиши код выше этой строки
